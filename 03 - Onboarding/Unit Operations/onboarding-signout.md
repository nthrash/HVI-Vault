---
title: Signout
description: Handoff workflows for HFICU night, Sarofim day, and PCCM APP coverage areas
---

# Signout

Our group covers three main areas, each with its own signout workflow:

| Coverage Area | Shift | APPs |
|---|---|---|
| Heart Failure ICU (HFICU) | Night only | 1-2 |
| Sarofim ICU/IMU | Day only | 1-2 |
| Pulmonary Critical Care (Sarofim ICU) | Day only | 1 |

---

## HFICU Night Shift

### Receiving Handoff (1900 - 1930)

1. **In-person signout with cardiology/HF fellows** — Meet in the office at 1900 to receive the day's handoff from the heart failure and cardiology fellows
2. **Update the HFICU Signout document** — Update the signout in real time during handoff (see [Shared Drive Access](onboarding-shared-drive-access.md) for file location)
3. **Phone signout with PCCM attending** — Call the daytime pulmonary critical care attending (usually already off campus) and run the list with them

### Giving Handoff (0600 - 0715)

#### Morning Text (0600)

Update the **Morning Text.docx** file (located in the Heart Failure folder on the shared drive) throughout the night. Send it out via **Epic Secure Chat** at **0600** to the following providers:

**Attending Physicians:**
- Dr. Manish Patel
- Dr. Carlos Manrique
- Dr. Harish Devineni
- Dr. Kha Dinh
- Dr. Bindu Akkanti
- Dr. Rahat Hussain
- Dr. Ismael Salas
- Dr. Sriram Nathan

**On-Call & Rounding Staff:**
- Rounding Cardiology Attending
- On-call Cardiology Attending
- AHF Night NP (current shift and next night)
- Daytime Cardiology/HF Fellow
- CVS/Organ Procurement 1st Call
- CVS PA/Fellows
- PCCM Day Attending
- PCCM Day Fellow

> On-call physicians can be found on the **On Call** tab of the HVI Nexus app. Make sure the correct date is selected at the top using the arrows. For night shift, select the date you **started** the shift, not the end date.

#### Surgery Rounds (0600 - 0645)

Round with **Dr. Salas** or **Dr. Marcano** (arrival time varies). Be ready with your overnight summary before they arrive as well as PTTs for patients on heparin gtts.

#### Morning Conference Call (0700)

- Dr. Nathan or the overnight attending will open with: *"Okay, let's get started — overnight events"*
- Go **room by room**: state the room number and patient name, then give a brief overnight summary
  - If nothing happened: *"No acute events"*
  - For **admissions**: preface with *"I'll let Dr. [attending] go over the details, but briefly..."* then give pertinent highlights — the overnight on-call cardiology attending will cover the full admission details after we finish the overnight events
- Stay on the call through the rest of the meeting until it ends (~0715)

#### Leaving the Unit

You may leave once a **cardiology fellow or heart failure fellow** is physically present on the unit.

---

## Sarofim Day Shift

<!-- TODO: Sarofim ICU/IMU day shift signout workflow -->

---

## PCCM APP (Sarofim ICU)

<!-- TODO: Pulmonary critical care APP signout workflow -->


---
_Last updated: March 2026 · HVI ICU APP Team_
