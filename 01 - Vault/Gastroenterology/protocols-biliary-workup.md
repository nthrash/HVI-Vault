---
title: "Hepatic Labs & RUQ Ultrasound in Heart Failure ICU: Cardiohepatic Interactions and Biliary Mimics"
category: "Cardiology / Critical Care"
audience: "APP/Physician"
created: "2026-05-15"
updated: "2026-05-16"
tags:
  - cardiology
  - heart-failure
  - hepatic-function
  - cardiohepatic-interactions
  - ultrasound
  - critical-care
  - icu-diagnosis
---

# Hepatic Labs & RUQ Ultrasound in Heart Failure ICU: Cardiohepatic Interactions and Biliary Mimics

## Table of Contents

1. [Overview](#overview)
2. [Part 1: Hepatic Lab Abnormalities in HF-ICU](#part-1-hepatic-lab-abnormalities-in-hf-icu)
3. [Part 2: RUQ Ultrasound Interpretation in HF-ICU](#part-2-ruq-ultrasound-interpretation-in-hf-icu)
4. [Clinical Pearl: Integrating Lab Findings and Ultrasound](#clinical-pearl-integrating-lab-findings-and-ultrasound)
5. [Clinical Pearls](#clinical-pearls)
6. [Evidence & Landmark Trials](#evidence--landmark-trials)
7. [Key Guidelines](#key-guidelines)
8. [References](#references)

---

## Overview

Abnormal hepatic laboratory tests are present in approximately **46% of patients hospitalized with acute decompensated heart failure (ADHF)** treated with inotropes (SURVIVE trial), and up to 50% of those requiring mechanical circulatory support (MCS). In the HF-ICU setting, these abnormalities arise not from primary liver disease, but from **cardiohepatic interactions** — the complex pathophysiology linking right ventricular failure, elevated systemic venous pressure, and impaired hepatic perfusion. Understanding this distinction is critical because it changes management: treating the heart (not the liver) is the primary intervention.  

Equally important is the ability to distinguish between true hepatobiliary pathology (acute cholecystitis, choledocholithiasis, pancreatitis) and **congestive mimics** that present with similar sonographic findings but require different management. This topic integrates the seven critical hepatic lab parameters, landmark cardiohepatic trials, and RUQ ultrasound interpretation into a unified clinical framework for HF-ICU practice. Encountering these findings is not an uncommon occurrence, thus we need to be able to distinguish these difference.

---

**Normals**

| Lab           | Normals | Unit  |
| ------------- | ------- | ----- |
| Amylase       | 30-110  | U/L   |
| Lipase        | 0-160   | U/L   |
| AST           | 10-40   | U/L   |
| ALT           | 7-56    | U/L   |
| Alp           | 44-147  | U/L   |
| Tbili         | 0.1-1.2 | mg/dL |
| Direct Bili   | 0-0.3   | mg/dL |
| Indirect Bili | 0.1-0.8 | mg/dL |
| LDH           | 140-280 | U/L   |
| Haptoglobin   | 30-200  | mg/dL |

---
## Part 1: Hepatic Lab Abnormalities in HF-ICU

### Etiology & Pathophysiology of Cardiohepatic Interactions

**Two-Hit Model of Acute Liver Injury in HF:**
1. **Hit 1: Hepatic Congestion** — Elevated right atrial pressure (RAP) causes venous backup into the hepatic sinusoids. Histologically, this results in centrilobular congestion and hepatocyte compression. The **portal vein pulsatility index (PVPI) >0.5** has 81% sensitivity and 87% specificity for congestion-driven hyperbilirubinemia.
2. **Hit 2: Hepatic Hypoperfusion** — Inadequate cardiac output (CI <2.2) reduces splanchnic perfusion, causing ischemic hepatitis. This is *not* hypotension alone — the two-hit mechanism requires both congestion AND hypoperfusion. Ischemic hepatitis (acute cardiogenic liver injury, or ACLI) is characterized by massive transaminase elevation (often >1000 U/L), rising over 1–3 days, with ALT:LDH ratio <1.5 (distinguishing it from viral hepatitis, where ALT:LDH >1.5).

**Hemodynamic Context Matters:**
- Pure congestion (elevated RAP, preserved CI) → cholestatic pattern (↑TBili, ↑ALP, ↑GGT)
- Pure shock (low CI, preserved RAP) → unlikely to cause hepatobiliary injury alone
- Two-hit (congestion + shock) → ACLI with mixed hepatocellular and cholestatic features

---
### Section 1: Total Bilirubin (TBili)

**Clinical Significance:**
TBili is an independent predictor of cardiovascular death and HF hospitalization in HFrEF patients, even after adjustment for NT-proBNP. It also independently predicts RV failure post-LVAD, making it prognostically important beyond its diagnostic role.

**Step 1: Fractionate — Direct vs. Indirect**
**Indirect TBili elevation (unconjugated):**
- Hemolysis: mechanical (LVAD, ECMO, prosthetic valves), DIC, MAHA (microangiopathic hemolytic anemia)
- Gilbert syndrome: benign, TBili rarely >6 mg/dL
- Hematoma resorption
- Drug effects (atazanavir)

Workup: CBC + differential, reticulocyte count, LDH, haptoglobin, direct Coombs test, review LVAD/ECMO hemolysis parameters.

**Direct TBili elevation (conjugated) — R Ratio:**
Calculate R Ratio = (ALT÷ULN) ÷ (ALP÷ULN):
- R >5 = hepatocellular pattern
- R 2–5 = mixed pattern
- R <2 = cholestatic pattern (typical in congestive hepatopathy)

**Step 2: Biliary Imaging**
RUQ ultrasound to evaluate for extrahepatic obstruction:
- Dilated ducts + stone → extrahepatic obstruction (MRCP or ERCP)
- Non-dilated ducts → intrahepatic cause (hemodynamic assessment, medication review, sepsis)

**Step 3: Hemodynamic Phenotyping**
Echo ± PA catheter:
- **PVPI >0.5**: 81% Sn, 87% Sp for congestion-driven TBili
- **Two-hit model**: ALT:LDH <1.5 indicates ischemic hepatitis (ACLI); peak d1–3, ≥50% fall within 72h of hemo stabilization

| Scenario                                                        | Early Phase (Hours–Days)                                                                                                                                                                                                                              | Late Phase (Days–Weeks)                                                                                                                                                                    | Fractionation Pattern                                                                                                                                | Key Distinguishing Features                                                                                                                                                                                         |
| --------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Ischemic hepatitis (shock liver)**                            | TBili rises modestly (usually 2–5 mg/dL), lagging behind the AST/ALT spike by 1–3 days. Direct bilirubin predominates due to hepatocellular dysfunction impairing canalicular excretion.                                                              | TBili peaks at days 3–7, then slowly declines. May remain elevated for 2–3 weeks due to delta bilirubin formation. AST/ALT normalize faster (7–10 days).                                   | **Mixed** — direct predominant but indirect component present from impaired conjugation in severe cases                                              | TBili rarely exceeds 10–15 mg/dL unless concurrent congestion or hemolysis. The AST/ALT:bilirubin dissociation (massive transaminases, modest bilirubin) is characteristic.                                         |
| **Hemolysis (Impella/ECMO)**                                    | Indirect bilirubin rises within hours of device initiation. TBili typically 5 mg/dL from hemolysis alone unless severe or concurrent renal/hepatic dysfunction.                                                                                       | Persists as long as device is running. Resolves within days of device removal if no other process.                                                                                         | **Indirect predominant** — direct bilirubin normal or near-normal if liver function intact                                                           | Confirmed by ↑LDH, ↓haptoglobin, ↑plasma free hemoglobin, ↑reticulocyte count. Hemolysis alone infrequently causes TBili >5 mg/dL unless coexistent renal or liver disease.                                         |
| **Hemolysis + ischemic injury (combined, as in ECPELLA shock)** | Both indirect AND direct bilirubin rise. TBili can reach 10–30+ mg/dL because overproduction overwhelms an already injured liver's conjugation and excretion capacity.                                                                                | Slow decline over weeks. Delta bilirubin formation prolongs jaundice well beyond resolution of the acute insult. CRRT does not clear bilirubin effectively (protein-bound).                | **Mixed** — both fractions elevated, often with indirect > direct early, shifting to direct > indirect as hepatocellular dysfunction dominates       | This is the pattern in the index patient. The key is that neither hemolysis nor ischemia alone explains TBili of 31 — it is the combination.                                                                        |
| **Acute RV failure / acute congestion**                         | TBili rises within 24–48h from sinusoidal distension compressing hepatocytes and impairing canalicular excretion. Direct bilirubin predominates.                                                                                                      | If congestion persists, TBili continues to rise. Correlates with CVP, TR severity, and right-sided filling pressures. Transition to cholestatic pattern (↑GGT, ↑ALP) over weeks.           | **Direct predominant**                                                                                                                               | Bilirubin is the most sensitive LFT marker of right-sided congestion — more sensitive than transaminases. Correlates with JVD, edema, TR grade.                                                                     |
| **Chronic congestive hepatopathy**                              | N/A (already established)                                                                                                                                                                                                                             | TBili chronically elevated (usually 2–8 mg/dL). Part of the cholestatic triad with ↑GGT and ↑ALP. Synthetic dysfunction (↑INR, ↓albumin) indicates progression toward cardiac cirrhosis.   | **Direct predominant**                                                                                                                               | Cholestatic LFTs (ALP, GGT, bilirubin) more common than transaminase elevation. Severity correlates with NYHA class and TR grade. Hyperbilirubinemia independently predicts mortality.                              |
| **Sepsis-associated cholestasis**                               | Conjugated bilirubin rises early — often within 24–48h of sepsis onset. May be the first biochemical abnormality. Inflammation downregulates canalicular excretion pumps (MRP2) and upregulates alternative transporters toward systemic circulation. | Sustained high bilirubin indicates complicated ICU course. Cholestatic markers (ALP, GGT) rise progressively. Bilirubin >10 mg/dL in sepsis carries poor prognosis.                        | **Direct predominant** — conjugated fractions of both bilirubin and bile acids are high, indicating the liver can still conjugate but cannot excrete | Cholestatic alterations occur earlier than traditionally recognized. The liver redirects bile acids and bilirubin toward the systemic circulation — this may be partially adaptive rather than purely pathological. |
| **TPN-related cholestasis**                                     | Conjugated bilirubin begins rising after ~7–14 days of TPN. ALP and GGT rise in parallel.                                                                                                                                                             | Progressive conjugated hyperbilirubinemia. TBili >3 mg/dL considered severe PNALD. Can progress to cirrhosis in prolonged cases (>6 months).                                               | **Direct predominant** — conjugated hyperbilirubinemia is the hallmark                                                                               | Multifactorial: absence of enteral stimulation, soybean lipid emulsions (phytosterols), glucose excess. Fish oil-based lipids may reduce risk. Enteral feeding is the most effective prevention.                    |
| **Biliary obstruction**                                         | Direct bilirubin rises within 24–48h. ALP may lag initially. TBili can exceed 20–30 mg/dL in complete obstruction.                                                                                                                                    | Persistent and progressive if obstruction not relieved. After relief, bilirubin declines slowly due to delta bilirubin (half-life ~3 weeks).                                               | **Direct predominant** (>50% direct)                                                                                                                 | Imaging (RUQ ultrasound) is mandatory. Dilated ducts confirm extrahepatic obstruction. In the cardiac ICU, consider acalculous cholecystitis as well.                                                               |
| **Delta bilirubin effect (resolving injury)**                   | Not present acutely — forms over days as conjugated bilirubin accumulates and binds albumin.                                                                                                                                                          | Dominates the bilirubin profile during recovery. Measured as "direct" bilirubin by standard assays but is NOT true conjugated bilirubin. Half-life ~17–21 days. Cannot be cleared by CRRT. | Appears as persistent **direct** hyperbilirubinemia on standard labs despite resolving liver injury                                                  | The most common cause of "why won't the bilirubin come down?" in recovering ICU patients. AST/ALT normalize, synthetic function improves, but bilirubin persists for weeks. Not a sign of ongoing injury.           |

---

### Section 2: Alkaline Phosphatase (ALP)

**Key Distinction: Is ALP Hepatic or Bone?**
Check GGT — it is NOT present in bone, making this the critical discriminator.
If ALP is elevated, check GGT to determine source (see Section 3). GGT elevated → hepatic. GGT normal → consider bone, pregnancy, intestinal.

**GGT elevated** → Hepatic ALP:
- Congestive hepatopathy (most common in HF-ICU): correlates with TR severity, RAP, right-sided pressures
- Drug-induced (amiodarone, antibiotics, TPN)
- Infiltrative (amyloidosis, sarcoidosis, hepatic mets)
- PBC (check AMA, ANA), PSC (check pANCA)
- Sepsis-associated cholestasis
- Biliary obstruction (RUQ ultrasound mandatory)

**GGT normal** → Non-hepatic ALP:
- Bone: Paget's disease, bony metastases, healing fracture
- Pregnancy, intestinal source (post-prandial)

**Low ALP:**
Uncommon but significant when present. Consider acute severe illness (ICU epiphenomenon), malnutrition, Zn or Mg deficiency, hypothyroidism, bisphosphonates, hypophosphatasia (rare genetic condition associated with low alkaline phosphatase, musculoskeletal symptoms, and dental anomalies — check pyridoxal-5-phosphate and phosphoethanolamine).

**Prognosis:**
The SURVIVE trial found that elevated ALP in ADHF predicted 180-day mortality of 34.9% vs. 23.5% (p=0.001) in patients without ALP elevation, reflecting the burden of systemic congestion.

---

### Section 3: Gamma-Glutamyl Transferase (GGT)

**Primary Role: Distinguishes cholestatic from non-cholestatic liver injury**
GGT is NOT present in bone, making it the only reliable way to distinguish hepatic from bone-derived ALP elevation when ALP is high.

**HF-Specific Interpretation:**
- GGT rises in response to **cholestatic stress** — bile duct injury, biliary obstruction, or chronic congestion causing canalicular damage. Cardiac ICU shock patient's liver injury is primarily **ischemic** (from cardiogenic shock) and **hemolytic** (from Impella/ECMO), neither of which directly injures bile ducts or canalicular transport in the acute phase.
- GGT + ALP + TBili = **cholestatic triad** of congestive hepatopathy
- Correlates with JVD, peripheral edema, TR grade, NYHA class, and right-sided filling pressures
- GGT is inversely associated with survival in stable HF patients

**Non-Cardiac Causes:**
- **Alcohol**: GGT elevated in >50% of heavy drinkers even without overt liver disease — most sensitive marker of alcohol intake
- Medications: phenytoin, barbiturates, carbamazepine
- Pancreatic disease, diabetes, renal failure
- NAFLD / NASH
- Biliary obstruction

| Scenario                                            | Early Phase                                                                                                                                            | Late Phase                                                                                                                          | Why                                                                                                                                                                                                                                                                                                             |
| --------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Ischemic hepatitis (shock liver)**                | GGT **normal**. AST/ALT spike massively (10–20× ULN) within 1–3 days. Bilirubin rises modestly.                                                        | GGT **remains normal**. AST/ALT normalize within 7–10 days if hemodynamics corrected. Bilirubin may lag.                            | Injury is zone 3 hepatocyte necrosis from hypoxia. Canaliculi and bile ducts are not involved. No bile acid accumulation → no GGT release.                                                                                                                                                                      |
| **Hemolysis (Impella/ECMO)**                        | GGT **normal**. Indirect bilirubin rises. LDH elevated. Haptoglobin low.                                                                               | GGT **remains normal** as long as hemolysis is the sole process.                                                                    | Bilirubin overproduction is pre-hepatic. No hepatic or biliary injury whatsoever.                                                                                                                                                                                                                               |
| **Acute RV failure / acute congestion**             | GGT **normal**. AST/ALT may rise modestly. Bilirubin rises from impaired hepatocellular clearance (sinusoidal distension).                             | GGT **begins to rise over weeks** if congestion persists. Transitions to cholestatic pattern (↑GGT, ↑ALP, ↑bilirubin).              | Early: sinusoidal distension compresses hepatocytes but canalicular apparatus is intact. Late: sustained elevated venous pressure → canalicular distension → bile stasis → bile acid accumulation → GGT solubilized from membrane.                                                                              |
| **Chronic congestive hepatopathy**                  | N/A (already established)                                                                                                                              | GGT **elevated**. ALP elevated. Bilirubin elevated. Cholestatic triad. Correlates with JVD, edema, TR grade.                        | Sustained congestion has caused chronic canalicular distension and bile stasis. Bile acids accumulate and solubilize GGT from the canalicular membrane. This is the hallmark biochemical pattern.                                                                                                               |
| **Biliary obstruction (e.g., choledocholithiasis)** | GGT **rises within hours to days** — intermediate rise (~4-fold by 24h), faster than ALP but slower than transaminases. ALP may still be normal at 6h. | GGT **markedly elevated** (often >10× ULN). ALP catches up. Bilirubin rises. Transaminases may actually fall while GGT/ALP persist. | Mechanical obstruction → immediate bile acid retention → rapid detergent effect on canalicular membrane. GGT rises earlier and persists longer than ALP in obstruction.                                                                                                                                         |
| **Sepsis-related cholestasis**                      | GGT may be **mildly elevated early** or normal. Conjugated bilirubin rises early (altered bile acid transport).                                        | GGT **rises progressively** if cholestasis persists. Sustained high cholestatic markers indicate complicated ICU course.            | Inflammation alters bile acid transporter expression — excretion pumps toward canaliculi are downregulated, alternative transporters toward systemic circulation are upregulated. Bile acid retention eventually causes GGT release, but the mechanism is intrahepatic cholestasis, not mechanical obstruction. |
| **Drug-induced cholestatic injury**                 | GGT **rises early** — often before ALP. Latency typically 2–12 weeks from drug start.                                                                  | GGT **persists longer than ALP** after drug discontinuation. Can remain elevated for months. Rarely → vanishing bile duct syndrome. | Drug-mediated injury to canalicular transport proteins or cholangiocytes. GGT rises earlier because it is more sensitive to early bile acid retention than ALP.                                                                                                                                                 |
| **Alcohol use**                                     | GGT **elevated** even without overt liver disease. Most sensitive marker of alcohol intake (>50% of heavy drinkers).                                   | GGT **remains elevated** with continued use. Normalizes with abstinence (half-life ~14–26 days).                                    | Alcohol induces GGT synthesis via microsomal enzyme induction (CYP2E1 pathway) AND causes oxidative stress — both independent of cholestasis. This is why GGT can be elevated with normal ALP/bilirubin in drinkers.                                                                                            |
| **NAFLD/NASH**                                      | GGT **mildly elevated** (usually 200 U/L).                                                                                                             | GGT **remains mildly elevated**. Correlates with ALT, AST, and triglycerides — not with ALP.                                        | Mechanism is oxidative stress and lipid peroxidation, NOT cholestasis. This is why GGT in NAFLD correlates with metabolic markers rather than cholestatic markers.                                                                                                                                              |


---

### Section 4: AST and ALT (Transaminases)

**Stratified by Degree of Elevation:**
**>20× ULN (Massive Elevation):**
- Acute cardiogenic liver injury (ACLI) from two-hit mechanism (congestion + hypoperfusion)
- Acetaminophen toxicity, acute viral hepatitis, Budd-Chiari syndrome

Key clues for ACLI:
- ALT:LDH <1.5 (distinguishes cardiogenic from viral hepatitis)
- Rapid rise over 1–3 days
- ≥50% fall in transaminases within 72 hours of hemodynamic stabilization
- ~80–84% show ≥50% drop in prothrombin activity (distinguishes from viral hepatitis)

Management: Restore cardiac output (inotropes, MCS escalation, ensure oxygenation).

**3–20× ULN (Moderate Elevation):**
Broader differential: DILI, viral hepatitis, autoimmune hepatitis, ischemic hepatitis with partial recovery, sepsis.

Workup: ALT:LDH ratio, hepatitis panel, ANA, ASMA, medication review.

**<3× ULN (Mild Elevation):**
Nonspecific: congestive hepatopathy, NAFLD/NASH, thyroid disease, celiac disease, strenuous exercise.

Context-dependent: if persistent or rising, pursue full workup; if isolated mild, monitor.

**Kinetics & Prognosis:**
ACLI shows rapid rise over 1–3 days and normalization within 7–10 days with hemodynamic stabilization. Failure to decline despite hemo improvement suggests ongoing ischemia, DILI, viral hepatitis, or primary liver disease. The SURVIVE trial found that elevated transaminases predicted 31-day mortality of 17.6% vs. 8.4% and 180-day mortality of 31.6% vs. 22.4%, reflecting hypoperfusion burden.

**Low AST/ALT:**
Pyridoxine (B6) deficiency (ALT is pyridoxal-phosphate dependent) or advanced sarcopenia/cachexia.
STRONG-HF trial finding that lower ALT in acute HF was paradoxically associated with **greater congestion** and a trend toward worse outcomes — likely reflecting sarcopenia and reduced hepatic functional reserve rather than hepatic health.

---

### Section 5: Lactate Dehydrogenase (LDH)

**Context-First Interpretation — LDH is Ubiquitous:**
LDH is released from any tissue undergoing ischemia, hemolysis, or inflammation. Always correlate with clinical picture.

**Acute Cardiogenic Liver Injury (ACLI):**
- LDH rises rapidly 10–20× ULN within 1–3 days
- ALT:LDH <1.5 → favors cardiogenic over viral hepatitis
- Often precedes or parallels transaminase elevation
- Falls ≥50% within 72 hours of hemodynamic stabilization

**Hemolysis (Mechanical):**
- ↑LDH + ↓haptoglobin + ↑indirect bilirubin + ↑reticulocytes
- LVAD, ECMO, mechanical valves, DIC

**Tissue Ischemia / Infarction:**
- MI: LDH-1 > LDH-2 (flipped isoenzyme pattern)
- PE: LDH-3 predominance
- Mesenteric ischemia: LDH-5

**Critical Distinction: LDH vs. Serum Lactate**

| Parameter              | LDH                                                             | Serum Lactate                           |
| ---------------------- | --------------------------------------------------------------- | --------------------------------------- |
| What it is             | Enzyme (tissue injury marker)                                   | Metabolite (hypoperfusion marker)       |
| Clinical use           | Identifies ischemic tissue                                      | Assesses perfusion/shock state          |
| Prognostic value in HF | Higher LDH = worse outcomes                                     | Trajectory predicts shock reversibility |
| Interpretation         | Single value informative; <br>Trending for monitoring hemolysis | Serial measurements more useful         |

In cardiogenic shock, serial lactate clearance predicts shock reversibility: early clearance indicates reversible shock; persistent elevation suggests refractory shock or impaired hepatic lactate metabolism.

**Prognosis:**
The GALACTIC-HF trial found that higher LDH independently predicted worse outcomes in HFrEF (Q4 vs Q1: HR 1.84 for HF event or CV death). LDH improves risk prediction when added to the PREDICT-HF model.

**Low LDH:**
Rare. LDH-A or LDH-B genetic deficiency (rare genetic conditions). Not actionable in the ICU.

---

### Section 6: Albumin

**Key Concept: Albumin Half-Life ~20 Days = Reflects Chronic Dysfunction**
In contrast to INR (t½ hours–days, reflects acute synthetic failure), albumin reflects chronic hepatic synthetic impairment, malnutrition, inflammation, and hemodilution.
Hypoalbuminemia is present in ~32% of HF patients.

**Contributing Mechanisms (Multifactorial):**
1. **Hepatic Congestion** → impaired synthesis
2. **Systemic Inflammation** → cytokine-mediated suppression of synthesis (primary driver, more significant than malnutrition alone)
3. **Malnutrition / Cardiac Cachexia** → inadequate substrate
4. **Hemodilution** → volume overload
5. **Protein-Losing Enteropathy** → gut edema from elevated RAP
6. **Nephrotic Syndrome** → urinary losses (check urine protein)

**Workup:**
- LFTs + INR (synthetic function panel)
- Urine protein / albumin:creatinine ratio
- CRP, ESR (inflammation burden)
- Nutritional assessment (prealbumin if available)
- Echo: IVC, RAP (hemodilution vs. true hepatic impairment)

**Management:**
- Optimize decongestion (reduces congestion + hemodilution simultaneously)
- Optimize oral nutrition (improved outcomes in malnourished HF patients)
- IV albumin: controversial; routine use does NOT improve outcomes. Consider selectively for refractory edema/ascites + profound hypoalbuminemia on max diuretics

**Elevated Albumin:**
Almost always dehydration or hemoconcentration. Reassess volume status.

**Prognosis:**
In-hospital mortality is markedly higher with hypoalbuminemia in acute HF (RR 4.90, 95% CI 2.96–8.10), with long-term mortality RR 1.75 (95% CI 1.35–2.27) — based on a meta-analysis of 9 studies (n=16,763; Peng et al., Biomarkers 2019). Hypoalbuminemia is an independent predictor of worse outcomes post-LVAD.

---

### Section 7: INR / Prothrombin Time

**Step 1: Exclude Anticoagulation First**
- Warfarin → INR elevation expected
- Anti-Xa DOACs may falsely elevate INR on some assays

**Step 2: Non-Anticoagulated Elevated INR — Three Most Common ICU Causes**
**1. Vitamin K Deficiency (39.6%):**
- Low FVII, FX, Protein C, Protein S
- Causes: poor nutrition, antibiotics, biliary obstruction
- Rx: Vitamin K IV or PO; plasma generally avoidable

**2. Liver Disease Pattern (35.4%):**
- Low FV, low or normal fibrinogen, high FVIII
- Indicates impaired hepatic synthetic function
- HF-specific: correlates with TBili, GGT, IVC diameter, TR severity, neurohormonal activation
- Rx: Treat underlying cause (decongestion in HF); plasma NOT appropriate for routine INR correction in liver disease

**3. Occult DOAC Effect (14.6%):**
- Anti-Xa DOAC detected on assay despite no reported anticoagulant use

**ACLI-Specific Finding:**
~80–84% of ACLI patients show ≥50% drop in prothrombin activity, distinguishing cardiogenic from viral hepatitis.

**Persistent Coagulopathy:**
Despite hemodynamic optimization → suggests progression to cardiac cirrhosis.

**Prognosis:**
INR is an independent predictor of all-cause mortality in ADHF (HR 1.89 per 0.1 increase) — higher prognostic value than MELD or MELD-XI. INR + TBili + Albumin together comprise the **hepatic synthetic function triad**.

---

### Synthetic Function Panel — Apply to All Patterns

| Parameter | Interpretation | Clinical Pearl |
|-----------|---|---|
| **Albumin** (t½ ~20 days) | Reflects CHRONIC dysfunction, inflammation, malnutrition, hemodilution | Low = congestion + inflammation + malnutrition; takes weeks to improve |
| **INR** (t½ hours–days) | Reflects ACUTE synthetic failure | INR >1.5 + encephalopathy = ALF criteria → hepatology + transplant consult |
| **MELD-Na** (TBili + INR + Cr + Na) | Prognostic score + transplant listing tool | Calculate on admission and trend serially |
| **Child-Pugh** (Albumin + INR + ascites + bili + encephalopathy) | Grades A/B/C hepatic reserve | Useful for cirrhosis assessment |
| **Platelets + Splenomegaly** | Reflects portal hypertension / cardiac cirrhosis | Low platelets + splenomegaly = suspect fibrosis progression |

**Pre-LVAD / Pre-Transplant Consideration:**
Persistent hyperbilirubinemia + abnormal imaging → liver biopsy may be required. Noninvasive tools (liver stiffness, MRE) cannot reliably differentiate congestion from fibrosis in this population.

---

## Part 2: RUQ Ultrasound Interpretation in HF-ICU

### The Critical Challenge: Gallbladder Wall Thickening is Nonspecific
Gallbladder wall thickening (≥3 mm) occurs in numerous non-cholecystitis conditions and is the most common source of diagnostic confusion in HF-ICU patients. The finding appears nearly expected in patients with CHF, hypoalbuminemia, and ascites, yet is often reflexively attributed to cholecystitis.

**Nonspecific Causes of GB Wall Thickening:**

| Cause | Mechanism | Key Distinguishing Features |
|-------|-----------|---|
| **Congestive Heart Failure** | Venous congestion, elevated RAP | Dilated IVC/hepatic veins, abnormal hepatic vein Doppler, NO stones, NO Murphy's sign, PVPI >0.5 |
| **Hypoalbuminemia** | Decreased oncotic pressure → wall edema | Diffuse stratified (layered) thickening, serum albumin <3 g/dL, NO stones, NO Murphy's sign |
| **Ascites** | Extrinsic fluid pressure on GB wall | Peritoneal fluid visible, wall thickening resolves with paracentesis |
| **Acute Hepatitis** | Inflammatory edema | Markedly elevated transaminases (often >1000), positive hepatitis serologies |
| **Portal Hypertension / Cirrhosis** | Venous congestion of GB wall | Stigmata of cirrhosis, splenomegaly, varices, reversed portal flow |
| **Renal Failure** | Volume overload, decreased oncotic pressure | Elevated creatinine, volume overloaded appearance |
| **Acute Cholecystitis** | Inflammatory infiltration, cystic duct obstruction | Stones + Murphy's sign + wall thickening ± focal striations (gangrenous changes) |

---

### Section 1: Acute Calculous Cholecystitis

**Sonographic Findings (Classic):**
- **Gallstones** — echogenic foci with posterior acoustic shadowing (95–99% of cases)
- **GB wall thickening ≥3–5 mm**
- **Pericholecystic fluid** — anechoic fluid surrounding gallbladder
- **Sonographic Murphy's sign** — focal tenderness over gallbladder with probe pressure (PPV 92% when combined with stones)
- **GB distension** — anteroposterior diameter >4 cm
- **Impacted stone at GB neck** — strongly associated with complicated cholecystitis

**Diagnostic Accuracy:**
- Stones + Murphy's sign: PPV 92%
- Stones + wall thickening: PPV 95%

**Important Caveat:**
Gallbladder wall changes (thickening, pericholecystic fluid) may not be present early — they can develop dynamically over a median of ~4 hours after presentation.

---

### Section 2: Acalculous Cholecystitis (AAC)

**The More Common Form in ICU (5–10% of cholecystitis, but disproportionately ICU):**
AAC is the predominant form of cholecystitis in critically ill patients. The challenge: many sonographic features overlap with findings routinely present in ICU patients (volume overload, hypoalbuminemia, systemic illness).

**SCCM Diagnostic Criteria:**

| Finding | Criterion |
|---------|-----------|
| GB wall thickening | >3 mm (some reports up to 9 mm) |
| GB distension (hydrops) | Short-axis diameter >40 mm |
| Pericholecystic fluid | Present |
| Sludge/echogenic bile | Minor criterion |
| Sonographic Murphy's sign | Often unreliable in sedated/intubated patients |
| Gallstones | **Absent** (defining feature) |

**Diagnostic Performance:**
A prospective ICU study requiring 3 major criteria (wall thickness >4 mm, hydrops, sludge) yielded:
- Specificity: 94%
- Sensitivity: 50%
- **A negative RUQ ultrasound does NOT rule out acalculous cholecystitis in the ICU.**

The SCCM guidelines emphasize that many sonographic features of AAC are routinely present in ICU patients from volume overload, hypoalbuminemia, etc., making diagnosis particularly difficult.

**Key Distinguishing Features from Calculous Cholecystitis:**
- NO stones (by definition)
- Sludge is more prominent
- Murphy's sign often unobtainable
- Clinical context critical: prolonged fasting, TPN, mechanical ventilation, vasopressors, severe systemic illness

---

### Section 3: Biliary Obstruction / Choledocholithiasis

**Primary Finding: CBD Dilation**
- **CBD diameter >6 mm** (with intact gallbladder) suggests obstruction as a general threshold
- **Post-cholecystectomy:** CBD dilates significantly; an upper limit of **8–10 mm** is more appropriate
- **Age adjustment:** The commonly cited rule of "add 1 mm per decade after age 60" is an approximation. A population-based MRCP study (n=938 healthy subjects, Beyer et al., Gut 2023) found that age-dependent 95th percentile upper reference limits were **up to 8 mm in subjects 65 years** and **up to 11 mm in subjects ≥65 years** — substantially higher than conventional thresholds. Using conventional cutoffs, 10.9% of healthy volunteers would have been falsely classified as having biliary dilation. An EUS study (n=647, Benjaminov et al., Surg Endosc 2013) confirmed significant CBD dilation after age 70, but even in the most elderly patients with intact gallbladder, normal CBD did not exceed 7.6 mm
- **Clinical implication:** Mild CBD dilation in elderly patients should be interpreted cautiously and correlated with clinical symptoms and LFTs before pursuing invasive workup. The AJR Expert Panel recommends that biliary duct dilation is unlikely to indicate obstruction in the absence of clinical symptoms or elevated LFTs (Ludwig et al., AJR 2024).
- **Intrahepatic biliary dilation** — "parallel channel" sign (dilated ducts running alongside portal veins)
- **CBD stone visualization** — echogenic focus within duct ± shadowing
- **Gallbladder distension** — Courvoisier sign if distal obstruction and GB intact

**Limitations:**
Ultrasound sensitivity for CBD stones only 50–75% (due to duodenal gas obscuration). If clinical suspicion is high despite negative ultrasound:
- MRCP: 84% sensitivity, 92% specificity
- Endoscopic ultrasound: 94% sensitivity, 89% specificity

---

### Section 4: Acute Pancreatitis

**Ultrasound as Initial Modality — Primarily to Identify Etiology**
Ultrasound is recommended to identify the underlying cause (gallstones) rather than to diagnose pancreatitis itself. The pancreas may appear sonographically normal in up to 23–27% of cases.

**When Abnormal:**
- **Enlarged, hypoechoic (edematous) pancreas** — classic finding of interstitial edematous pancreatitis
- **Peripancreatic edema/fluid** — most frequently identified (up to 63%)
- **Pancreatic duct dilation** — cited as reliable but identified in only ~12–14%
- **Peripancreatic fluid collections** — anechoic or complex
- **Gallstones in GB or CBD** — identifying etiology
- **CBD dilation** — may indicate passed or impacted stone

**Limitation:**
Bowel gas frequently obscures pancreas; ultrasound may underestimate extrapancreatic complications. CT with IV contrast is gold standard for assessing severity, necrosis, and complications.

---

### Section 5: Congestive Hepatopathy (The Critical HF-ICU Mimic)

**Most Important Entity to Distinguish from Primary Biliary/GB Disease:**
This is the diagnosis most often mimicked in HF-ICU patients. Characteristic findings:
- **Dilated IVC** with reduced or absent respiratory variation
- **Dilated hepatic veins** (>1 cm)
- **Loss of normal triphasic hepatic venous waveform** → monophasic or reversed flow on Doppler
- **Portal vein pulsatility** — abnormal pulsatile flow (PVPI >0.5) reflects transmitted right-sided pressures
- **Hepatomegaly** with heterogeneous parenchymal echotexture
- **Ascites** (often SAAG ≥1.1, supporting cardiac/portal hypertensive etiology)
- **Gallbladder wall thickening** — from venous congestion and/or hypoalbuminemia, NOT cholecystitis

**Doppler Key — Hepatic Venous Waveform:**
Normal: triphasic (S, D, A waves reflecting systolic, diastolic, and atrial components)
Abnormal in CHF: monophasic or completely reversed (indicating elevated RAP transmitted back through IVC → hepatic veins)

---

## Clinical Pearl: Integrating Lab Findings and Ultrasound

### Quick Decision Framework

**When confronted with abnormal hepatic labs + GB wall thickening:**
1. **Presence/absence of gallstones?** → Largest decision point
   - Stones present + Murphy's sign → calculous cholecystitis
   - No stones + wall thickening → consider AAC vs. congestive mimic
2. **Hepatic vein and IVC Doppler pattern?**
   - Abnormal (monophasic/reversed) → congestion is likely explanation
   - Normal → primary biliary disease more likely
3. **Clinical context?**
   - Fever, leukocytosis, RUQ pain → favor cholecystitis
   - No fever, no pain, stable WBC → favor congestive mimic
4. **Synthetic function?**
   - Low albumin + high INR + high TBili → hepatic congestion pattern
   - Isolated transaminase elevation → ACLI if ALT:LDH <1.5
5. **HIDA scan?**
   - Equivocal ultrasound + clinical uncertainty → hepatobiliary scintigraphy shows GB filling (rules out AAC)
   - Limited value in prolonged fasting or severe hepatic dysfunction

---

## Evidence & Landmark Trials

### Cardiohepatic Interactions
**Samsky MD et al. (JACC 2013):** Cardiohepatic Interactions in Heart Failure
- Seminal framework establishing the pathophysiology of liver dysfunction in HF
- Introduced the concept of congestive hepatopathy and its prognostic implications

**Xanthopoulos A et al. (JACC: HF 2019):** Heart Failure and Liver Disease
- Comprehensive review of cardiohepatic pathophysiology in modern HF therapy
- Established prevalence of abnormal LFTs and their association with poor outcomes

**Styczynski G et al. (2016):** Portal Vein Pulsatility and Hepatic Congestion in HF
- Demonstrated PVPI >0.5: 81% sensitivity, 87% specificity for congestion-driven hyperbilirubinemia
- Validated portal vein Doppler as a hemodynamic assessment tool reflecting right-sided congestion

### Hepatic Markers in ADHF
**Nikolaou M et al. (Eur Heart J 2013, SURVIVE Trial):** LFT Abnormalities in ADHF
- Elevated ALP: 180-day mortality 34.9% vs 23.5% (p=0.001)
- Elevated transaminases: 31-day mortality 17.6% vs 8.4%; 180-day mortality 31.6% vs 22.4%
- Established independent prognostic value of hepatic markers

**Samsky MD et al. (Eur J Heart Fail 2016, ASCEND-HF):** LFTs and Outcomes in Acute HF
- Post hoc analysis of 4,228 ADHF patients with complete admission LFT data
- 42% had abnormal bilirubin, 22% abnormal ALT, 30% abnormal AST at admission
- After multivariable adjustment, **only elevated bilirubin** was independently associated with worse outcomes:
    - 30-day mortality or HF rehospitalization: HR 1.17 per 1 mg/dL increase (95% CI 1.04–1.32, p=0.012)
    - Abnormal bilirubin vs. normal: HR 1.24 for 30-day composite (95% CI 1.00–1.54, p=0.048); HR 1.32 for 180-day mortality (95% CI 1.08–1.62, p=0.007)
- **AST and ALT were NOT independently associated with outcomes** after adjustment — contrasting with SURVIVE (which enrolled sicker patients requiring inotropes)
- Clinical implication: In less severe ADHF (without cardiogenic shock), bilirubin is the dominant prognostic hepatic marker, while transaminase elevation may reflect reversible hemodynamic perturbation

**Okada A et al. (Circ J 2016):** Prognostic Value of INR in ADHF
- INR: independent predictor of all-cause mortality (HR 1.89 per 0.1 increase)
- Higher prognostic value than MELD or MELD-XI

**Ono R et al. (JACC: HF 2026, GALACTIC-HF):** LDH and Outcomes in HFrEF
- Higher LDH independently predicts worse outcomes (Q4 vs Q1: HR 1.84, 95% CI 1.62–2.08)
- LDH remains independently associated with outcomes after adjustment; improves PREDICT-HF risk model (Harrell's C statistic, IDI, NRI all improved)

**Peng W et al. (Biomarkers 2019):** Hypoalbuminemia and Mortality in HF — Meta-Analysis (9 studies, n=16,763)
- Hypoalbuminemia associated with in-hospital mortality RR 4.90 (95% CI 2.96–8.10) in acute HF
- Long-term all-cause mortality RR 1.75 (95% CI 1.35–2.27) in acute HF; 3.5-fold higher risk in chronic HF

**Suzuki K et al. (Eur J HF 2020, PARADIGM-HF):** Sacubitril/Valsartan Effects on LFTs
- Sacubitril/valsartan reduced TBili by 2.4% vs enalapril
- Suggests that improved neurohormonal biology reduces hepatic congestion

### RUQ Ultrasound in Cholecystitis
**Strasberg SM (NEJM 2008):** Acute Calculous Cholecystitis
- Classic review of sonographic findings and diagnostic accuracy
- Stones + Murphy's sign: PPV 92%

**Ivanov D et al. (J Emerg Med 2025):** Evolution of Secondary Findings in Acute Cholecystitis
- Wall changes may develop dynamically over median ~4 hours
- Emphasizes importance of serial imaging if initial study equivocal

**Mariat G et al. (Intensive Care Med 2000):** Acalculous Cholecystitis in ICU
- Prospective study showing specificity 94%, sensitivity 50% for 3-criteria SCCM approach
- Highlights difficulty of AAC diagnosis in ICU setting

---

## Key Guidelines
**ACC/AHA Guidelines for Heart Failure (2022):**
- Recommend assessment of liver function at baseline and during optimization in all HF patients
- Highlight prognostic importance of hepatic markers

**Society of Critical Care Medicine (SCCM) Bedside Ultrasound Guidelines (2015):**
- Diagnostic criteria for acalculous cholecystitis in critical care
- Emphasis on clinical correlation given overlap with ICU-related findings

**American College of Emergency Physicians (ACEP) Ultrasound Guidelines (2023):**
- Recommendations for RUQ ultrasound in acute biliary disease
- Diagnostic accuracy of gallstone detection and Murphy's sign

---

## Clinical Pearls
1. **Gallbladder wall thickening in HF-ICU is almost expected** — the combination of CHF, hypoalbuminemia, and ascites frequently coexist. Wall thickening alone should NOT reflexively be attributed to cholecystitis. Always integrate stone presence, Murphy's sign, hepatic vein Doppler, and clinical context.
2. **ALT:LDH <1.5 is your friend in ACLI** — when transaminases are markedly elevated and you need to distinguish cardiogenic from viral hepatitis, this ratio is more reliable than absolute LDH level. Ischemic hepatitis shows ALT:LDH <1.5; viral hepatitis typically >1.5.
3. **Albumin t½ ~20 days — plan for long recovery** — don't expect rapid improvement with decongestion alone. Nutritional optimization and time are required; IV albumin is controversial and doesn't improve outcomes routinely.
4. **INR >1.5 + encephalopathy in HF is ALF until proven otherwise** — this is the one time you call hepatology emergently. Acute Liver Failure criteria require both INR >1.5 AND hepatic encephalopathy in the absence of pre-existing cirrhosis. Don't miss this.
5. **The two-hit model explains why some ADHF patients develop ACLI and others don't** — congestion alone (elevated RAP) causes cholestasis. Shock alone (low CI) doesn't cause hepatobiliary injury. You need BOTH. Watch for ALT:LDH <1.5 as the tip-off.
6. **Murphy's sign is often unobtainable in sedated/vented patients** — don't dismiss AAC just because you can't elicit tenderness. Rely more heavily on imaging criteria and clinical context.
7. **HIDA scan has limited value in HF-ICU** — prolonged fasting + severe hepatic congestion reduce sensitivity. Use when ultrasound is truly equivocal and clinical suspicion is high, but understand the limitations.
8. The AHA scientific statement on right-sided heart failure specifically identifies elevated GGT, ALP, and bilirubin as the hallmark cholestatic markers of **chronic congestive hepatopathy** — the pattern seen with sustained elevated right-sided filling pressures over time. However, **acute cardiogenic liver injury** — which is what this patient experienced — has a fundamentally different biochemical profile. Literature suggests often times GGT will remains unchanged.

---

## References

### Landmark Trials & Key Reviews
1. Samsky MD, Patel CB, DeWald TA, et al. Cardiohepatic Interactions in Heart Failure: An Overview and Clinical Implications. *Journal of the American College of Cardiology*. 2013;61(24):2397-2405. doi:10.1016/j.jacc.2013.03.047.
2. Xanthopoulos A, Starling RC, Kitai T, Triposkiadis F. Heart Failure and Liver Disease: Cardiohepatic Interactions. *JACC: Heart Failure*. 2019;7(2):87-97. doi:10.1016/j.jchf.2018.10.007.
3. Nikolaou M, Parissis J, Yilmaz MB, et al. Liver Function Abnormalities, Clinical Profile, and Outcome in Acute Decompensated Heart Failure. *European Heart Journal*. 2013;34(10):742-749. doi:10.1093/eurheartj/ehs332.
4. Okada A, Sugano Y, Nagai T, et al. Prognostic Value of Prothrombin Time International Normalized Ratio in Acute Decompensated Heart Failure. *Circulation Journal*. 2016;80(4):913-923. doi:10.1253/circj.CJ-15-1326.
5. Ono R, Chimura M, Docherty KF, et al. Lactate Dehydrogenase and Outcomes in Patients With HF and Reduced Ejection Fraction: Insights From GALACTIC-HF. *JACC: Heart Failure*. 2026;14(5):102900. doi:10.1016/j.jchf.2025.102900.
6. Peng W, Zhang C, Wang Z, Yang W. Prediction of all-cause mortality with hypoalbuminemia in patients with heart failure: a meta-analysis. *Biomarkers*. 2019;24(7):631-637. doi:10.1080/1354750X.2019.1652686.
7. Suzuki K, Horie T, Yamamoto M, et al. Effects of Sacubitril/Valsartan on Liver Function Tests in Patients with Heart Failure with Reduced Ejection Fraction. *European Journal of Heart Failure*. 2020;22(9):1662-1671. doi:10.1002/ejhf.1947.

### Acute Cholecystitis & RUQ Ultrasound
9. Strasberg SM. Acute Calculous Cholecystitis. *The New England Journal of Medicine*. 2008;358(26):2804-2811. doi:10.1056/NEJMcp0800929.
10. Ivanov D, Cannata D, Chin KA, et al. Evolution of Secondary Findings in Acute Cholecystitis: A Temporal Analysis from Point-of-Care Ultrasound to Subsequent Imaging. *The Journal of Emergency Medicine*. 2025;78:266-274. doi:10.1016/j.jemermed.2025.03.020.
11. Mariat G, Mahul P, Prévôt N, et al. Contribution of Ultrasonography and Cholescintigraphy to the Diagnosis of Acute Acalculous Cholecystitis in Intensive Care Unit Patients. *Intensive Care Medicine*. 2000;26(11):1658-1663. doi:10.1007/s001340000684.
12. American College of Emergency Physicians (ACEP). Ultrasound Guidelines: Emergency, Point-of-Care and Clinical Ultrasound Guidelines in Medicine. *American College of Emergency Physicians*. 2023.
13. Frankel HL, Kirkpatrick AW, Elbarbary M, et al. Guidelines for the Appropriate Use of Bedside General and Cardiac Ultrasonography in the Evaluation of Critically Ill Patients: Part I. Critical Care Medicine. 2015;43(11):2479-2502. doi:10.1097/CCM.0000000000001216.

### Congestive Hepatopathy & Imaging
14. Wells ML, Fenstad ER, Poterucha JT, et al. Imaging Findings of Congestive Hepatopathy. *Radiographics*. 2016;36(4):1024-1037. doi:10.1148/rg.2016150207.
15. Ohara H, Yoshihisa A, Ishibashi S, et al. Hepatic Venous Stasis Index Reflects Hepatic Congestion and Predicts Adverse Outcomes in Patients with Heart Failure. *Journal of the American Heart Association*. 2023;12(12):e029857. doi:10.1161/JAHA.122.029857.
16. Stassen J, Falter M, Herbots L, et al. Assessment of Venous Congestion Using Vascular Ultrasound. *JACC: Cardiovascular Imaging*. 2023;16(3):426-431. doi:10.1016/j.jcmg.2022.12.028.
17. Giangregorio F, Centenara E, Mazzocchi S, et al. Assessing Venous Congestion in Acute and Chronic Heart Failure: A Review of Splanchnic, Cardiac and Pulmonary Ultrasound: Part 1. *Journal of Clinical Medicine*. 2025;14(22):8147. doi:10.3390/jcm14228147.

### Gallbladder Wall Thickening — Nonspecific Causes
18. Teefey SA, Baron RL, Bigler SA. Sonography of the Gallbladder: Significance of Striated (Layered) Thickening of the Gallbladder Wall. *AJR. American Journal of Roentgenology*. 1991;156(5):945-947. doi:10.2214/ajr.156.5.2017956.
19. Shlaer WJ, Leopold GR, Scheible FW. Sonography of the Thickened Gallbladder Wall: A Nonspecific Finding. *AJR. American Journal of Roentgenology*. 1981;136(2):337-339. doi:10.2214/ajr.136.2.337.
20. Wegener M, Börsch G, Schneider J, et al. Gallbladder Wall Thickening: A Frequent Finding in Various Nonbiliary Disorders. *Journal of Clinical Ultrasound*. 1987;15(5):307-312. doi:10.1002/jcu.1870150503.
21. Gupta P, Marodia Y, Bansal A, et al. Imaging-Based Algorithmic Approach to Gallbladder Wall Thickening. *World Journal of Gastroenterology*. 2020;26(40):6163-6181. doi:10.3748/wjg.v26.i40.6163.

### Current Society Guidelines
22. Heidenreich PA, Bozkurt B, Aguilar D, et al. 2022 AHA/ACC/HFSA Guideline for the Management of Heart Failure. *Journal of the American College of Cardiology*. 2022;79(17):e263-e421. doi:10.1016/j.jacc.2021.12.011.
23. Patel H, Jepsen J. Gallstone Disease: Common Questions and Answers. *American Family Physician*. 2024;109(6):518-524.
24. Mederos MA, Reber HA, Girgis MD. Acute Pancreatitis. *JAMA*. 2021;325(4):382-390. doi:10.1001/jama.2020.20317.

---

Created: 2026-05-15 · Last updated: 2026-05-16 · HVI ICU APP Team
